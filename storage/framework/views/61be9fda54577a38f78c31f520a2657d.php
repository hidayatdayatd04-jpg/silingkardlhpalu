...
<div class="space-y-6" wire:poll.30s>
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
            <div class="flex items-center gap-3">
                <h3 class="font-bold text-slate-800 dark:text-slate-200"><?php echo e(__('Daftar Armada')); ?></h3>
                <span class="flex items-center gap-1.5 px-2.5 py-0.5 bg-emerald-100 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 rounded-full text-xs font-bold"><span class="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></span><?php echo e($this->getActiveCount()); ?> <?php echo e(__('unit')); ?></span>
            </div>
            <p class="text-xs text-slate-500 mt-1"><?php echo e(__('Menampilkan seluruh armada beserta status mesin (menyala / parkir).')); ?></p>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($this->getLastSync()): ?><p class="text-xs text-slate-400 dark:text-slate-500 mt-1"><svg xmlns="http://www.w3.org/2000/svg" style="display:inline;width:14px;height:14px;vertical-align:middle" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> <?php echo e(__('Sinkronisasi terakhir:')); ?> <?php echo e($this->getLastSync()); ?></p><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        </div>
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="<?php echo e(__('Cari nama armada...')); ?>" class="w-full md:w-64 px-4 py-2 border border-slate-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 rounded-xl focus:ring-2 focus:ring-brand-500 text-sm"/>
    </div>
    <div class="w-full rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-2 shadow-sm relative">
        <div wire:ignore x-data
             x-init="setTimeout(function(){dlhMapInit('dlh-tracking-map');dlhMapDrawMarkers(<?php echo \Illuminate\Support\Js::from($this->getVehicles())->toHtml() ?>,true)},100)">
        ...
        <div class="p-4 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 rounded-b-2xl">
            <h4 class="font-bold text-xs text-slate-500 dark:text-slate-400 mb-4 uppercase tracking-wider text-center md:text-left"><?php echo e(__('Legenda Armada')); ?></h4>
            <div class="flex flex-col md:flex-row items-center justify-center md:justify-start gap-8">
                <div class="flex items-center gap-3"><div class="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700"><img src="/assets/tracking/car_acc_on.png" class="w-7 h-7 object-contain" alt="Pickup"></div><span class="text-sm font-bold text-slate-700 dark:text-slate-200"><?php echo e(__('Mobil Pickup (Roda 4)')); ?></span></div>
                <div class="flex items-center gap-3"><div class="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700"><img src="/assets/tracking/truck_acc_on.png" class="w-7 h-7 object-contain" alt="Dump Truck"></div><span class="text-sm font-bold text-slate-700 dark:text-slate-200"><?php echo e(__('Dump Truck (Roda 6)')); ?></span></div>
                <div class="flex items-center gap-3"><div class="w-12 h-12 flex items-center justify-center bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700"><img src="/assets/tracking/car_parking.png" class="w-7 h-7 object-contain" alt="Parkir"></div><span class="text-sm font-bold text-slate-700 dark:text-slate-200"><?php echo e(__('Parkir / Mesin Mati')); ?></span></div>
            </div>
        </div>
    </div>
</div><?php /**PATH D:\Backup\DLH - Palu\storage\framework\views/livewire/views/caa38d66.blade.php ENDPATH**/ ?>