<?php $__env->startSection('title', 'Profil Dinas - Dinas Lingkungan Hidup Kota Palu'); ?>
<?php $__env->startSection('description', 'Visi, misi, struktur organisasi, serta tugas dan fungsi Dinas Lingkungan Hidup Kota Palu.'); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.public.page-hero','data' => ['title' => ''.e(__('Profil Dinas Lingkungan Hidup')).'','description' => ''.e(__('Informasi resmi mengenai visi, misi, struktur organisasi, serta tugas dan fungsi Dinas Lingkungan Hidup Kota Palu.')).'','badge' => ''.e(__('Profil Instansi')).'']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['title' => ''.e(__('Profil Dinas Lingkungan Hidup')).'','description' => ''.e(__('Informasi resmi mengenai visi, misi, struktur organisasi, serta tugas dan fungsi Dinas Lingkungan Hidup Kota Palu.')).'','badge' => ''.e(__('Profil Instansi')).'']); ?>
...
<?php endif; ?>
    ...
    <section class="grid gap-4 sm:grid-cols-3">
        <button type="button" @click="setTab('visi-misi')" class="group rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900">
            ...
            <span class="mt-4 block text-base font-bold text-slate-900 dark:text-white"><?php echo e(__('Visi & Misi')); ?></span>
            <span class="mt-1 block text-sm leading-6 text-slate-500 dark:text-slate-400"><?php echo e(__('Arah pembangunan dan komitmen pelayanan lingkungan hidup.')); ?></span>
        </button>
        ...
        <button type="button" @click="setTab('struktur')" class="group rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900">
            ...
            <span class="mt-4 block text-base font-bold text-slate-900 dark:text-white"><?php echo e(__('Struktur Organisasi')); ?></span>
            <span class="mt-1 block text-sm leading-6 text-slate-500 dark:text-slate-400"><?php echo e(__('Ruang khusus untuk bagan organisasi yang akan ditambahkan dari asset.')); ?></span>
        </button>
        ...
        <button type="button" @click="setTab('tugas')" class="group rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-brand-300 hover:shadow-lg dark:border-slate-800 dark:bg-slate-900">
            ...
            <span class="mt-4 block text-base font-bold text-slate-900 dark:text-white"><?php echo e(__('Tugas & Fungsi')); ?></span>
            <span class="mt-1 block text-sm leading-6 text-slate-500 dark:text-slate-400"><?php echo e(__('Uraian kewenangan, bidang kerja, dan fungsi perangkat daerah.')); ?></span>
        </button>
    </section>
    ...
    <div class="sticky top-16 z-20 rounded-2xl border border-slate-200 bg-white/90 p-1.5 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
        <nav class="grid grid-cols-3 gap-1" aria-label="<?php echo e(__('Tab profil dinas')); ?>">
            <button type="button" @click="setTab('visi-misi')" :class="tab === 'visi-misi' ? 'bg-brand-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'" class="rounded-xl px-3 py-3 text-xs font-bold transition sm:text-sm"><?php echo e(__('Visi & Misi')); ?></button>
            <button type="button" @click="setTab('struktur')" :class="tab === 'struktur' ? 'bg-brand-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'" class="rounded-xl px-3 py-3 text-xs font-bold transition sm:text-sm"><?php echo e(__('Struktur')); ?></button>
            <button type="button" @click="setTab('tugas')" :class="tab === 'tugas' ? 'bg-brand-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'" class="rounded-xl px-3 py-3 text-xs font-bold transition sm:text-sm"><?php echo e(__('Tugas')); ?></button>
        </nav>
    </div>
    ...
    <section x-show="tab === 'visi-misi'" x-cloak id="visi-misi" class="space-y-6">
        <article class="profile-visi-card relative overflow-hidden rounded-3xl p-8 text-white sm:p-10 lg:p-12">
            ...
            <div class="relative flex flex-col items-center text-center">
                ...
                <p class="mt-4 text-xs font-bold uppercase tracking-[0.25em] text-emerald-300"><?php echo e(__('Visi')); ?></p>
                <div class="profile-vision mt-5">
                    <?php echo $profil->visi_translated; ?>
        ...
        <article class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-slate-200/50 dark:border-slate-800/80 dark:bg-slate-900 dark:shadow-slate-900/50 sm:p-8 lg:p-10">
            <div class="flex flex-col items-center border-b border-slate-100 pb-10 text-center dark:border-slate-800">
                <p class="text-xs font-bold uppercase tracking-[0.25em] text-emerald-600 dark:text-emerald-400"><?php echo e(__('Misi')); ?></p>
            </div>
            <div class="profile-prose profile-mission mt-7">
                <?php echo $profil->misi_translated; ?>
    ...
    <section x-show="tab === 'struktur'" x-cloak id="struktur-organisasi" class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:p-8">
        <div class="mb-6 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <p class="text-xs font-bold uppercase tracking-[0.22em] text-brand-600 dark:text-brand-300"><?php echo e(__('Struktur Organisasi')); ?></p>
                <h2 class="mt-2 text-2xl font-extrabold text-slate-900 dark:text-white"><?php echo e(__('Bagan Organisasi DLH Kota Palu')); ?></h2>
            </div>
        </div>
        ...
        <div class="flex min-h-[380px] items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center dark:border-slate-700 dark:bg-slate-950/40">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($structureImage): ?>
                <img src="<?php echo e($structureImage); ?>" alt="<?php echo e(__('Struktur Organisasi DLH Kota Palu')); ?>" loading="lazy" class="max-h-[720px] max-w-full rounded-xl object-contain">
            <?php else: ?>
                <div class="max-w-md">
                    ...
                    <h3 class="mt-5 text-lg font-extrabold text-slate-900 dark:text-white"><?php echo e(__('Gambar struktur belum tersedia')); ?></h3>
                    <p class="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400"><?php echo e(__('Area ini sengaja dikosongkan dulu agar nanti bisa langsung diisi dengan gambar struktur organisasi dari folder asset.')); ?></p>
                </div>
    ...
    <section x-show="tab === 'tugas'" x-cloak id="tugas-fungsi" class="rounded-3xl border border-slate-200/80 bg-white p-6 shadow-lg shadow-slate-200/50 dark:border-slate-800/80 dark:bg-slate-900 dark:shadow-slate-900/50 sm:p-10">
        <div class="mb-10 max-w-3xl">
            <p class="text-xs font-bold uppercase tracking-[0.25em] text-brand-600 dark:text-brand-300"><?php echo e(__('Tugas & Fungsi')); ?></p>
        </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Backup\DLH - Palu\resources\views/public/profil.blade.php ENDPATH**/ ?>