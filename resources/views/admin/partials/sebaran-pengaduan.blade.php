    ...
    <x-admin.card :padding="false" class="overflow-hidden">
        ...
        <div class="pl-filter">
            ...
            <p id="sp-range-text" class="pl-range-text">
                Menampilkan laporan <b>{{ \Carbon\Carbon::parse($from ?? now()->startOfMonth()->toDateString())->format('d M Y') }}</b>
                s.d.
                <b>{{ \Carbon\Carbon::parse($to ?? now()->endOfMonth()->toDateString())->format('d M Y') }}</b>
            </p>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            function loadReports(from, to, fit) {
                currentFrom = from;
                currentTo = to;
                var url = '{{ route("admin.peta-laporan.data") }}?from=' + encodeURIComponent(from) + '&to=' + encodeURIComponent(to);
