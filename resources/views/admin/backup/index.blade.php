...
@section('content')
        ...
        <x-slot:actions>
            <form method="POST" action="{{ route('admin.backup.store') }}" class="flex"
                ...
                <button type="submit" :disabled="loading"
                    class="relative inline-flex items-center justify-center gap-2 rounded-lg bg-brand-600 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-brand-600/30 transition hover:bg-brand-700 focus:outline-none focus:ring-4 focus:ring-brand-100 disabled:opacity-60">
                    <svg x-show="loading" x-cloak class="size-4 animate-spin" viewBox="0 0 24 24" fill="none">
    ...
    <div class="mt-4">
        <x-admin.alert type="warning">
            <strong>Perhatian:</strong> Restore akan <strong>menimpa seluruh data</strong> dengan isi file backup. Pre-restore otomatis dibuat.
        </x-admin.alert>
    </div>
                            ...
                            <div class="group flex items-center gap-3 px-4 py-2.5 transition hover:bg-brand-50/30">
                                ...
                                <x-admin.modal name="restore-{{ $loop->index }}" title="Restore Database" variant="danger" maxWidth="md">
                                    <div class="mb-4 flex items-start gap-3 rounded-xl bg-danger-50 p-4">
                                        ...
                                        <p class="text-sm text-danger-800">Restore dari <strong class="font-mono">{{ $b['name'] }}</strong>? Seluruh data saat ini akan <strong>ditimpa</strong>. Pastikan Anda telah memahami risikonya.</p>
                                    </div>
            ...
            <x-admin.card class="relative overflow-hidden">
                ...
                <div class="relative">
                    ...
                    <form id="restore-upload-form" method="POST" action="{{ route('admin.backup.restore') }}" enctype="multipart/form-data"
                        x-data="{ confirm: '', fileName: '', loading: false }" x-on:submit="loading = true" class="space-y-3">
                        @csrf
                        <div x-data="{ over: false }"
                            ...
                            <template x-if="fileName && !loading">
                                <div>
                            ...
                            <template x-if="loading">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-xs font-bold text-brand-700">Memproses restore...</p>
                                    <p class="text-[11px] text-slate-500">Pre-restore & eksekusi sedang berjalan. Jangan tutup halaman ini.</p>
                                </div>
                            </template>
                        </div>
                        ...
                        <div>
                            ...
                            <input name="confirmation" x-model="confirm" autocomplete="off" :disabled="loading"
                                class="h-9 w-full rounded-lg border border-slate-300 bg-white px-3 text-xs outline-none transition focus:border-danger-500 focus:ring-4 focus:ring-danger-100 disabled:opacity-50">
                        </div>

                        <button type="submit"
                            x-bind:disabled="confirm !== 'RESTORE' || !fileName || loading"
                            x-bind:class="(!loading && confirm === 'RESTORE' && fileName) ? 'bg-danger-600 hover:bg-danger-700 text-white shadow-[0_8px_20px_-8px_rgba(225,29,72,0.6)]' : 'bg-slate-200 text-slate-400 cursor-not-allowed'"
                            ...
                            <span x-text="loading ? 'Sedang diproses...' : 'Restore dari File'"></span>
                        </button>
                    </form>
                </div>
            </x-admin.card>
                ...
                <div class="flex items-start gap-2.5">
                    ...
                    <div>
                        ...
                        <ul class="mt-1.5 space-y-1 text-[11px] leading-relaxed text-slate-500">
                            <li class="flex gap-1.5"><x-admin.icon name="check" :size="12" class="mt-px shrink-0 text-emerald-500" />Lakukan backup secara berkala.</li>
                            <li class="flex gap-1.5"><x-admin.icon name="check" :size="12" class="mt-px shrink-0 text-emerald-500" />Simpan salinan di luar server.</li>
                            <li class="flex gap-1.5"><x-admin.icon name="check" :size="12" class="mt-px shrink-0 text-emerald-500" />Pre-restore otomatis dibuat.</li>
                        </ul>
                    </div>
                </div>
            </x-admin.card>
