...
<label class="flex cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-1.5 transition hover:bg-slate-50">
    <input 
        type="checkbox" 
        value="{{ $value }}"
        {{ $checked ? 'checked' : '' }}
    ...
    >
    <span class="flex-1 text-[13px] font-medium text-slate-700">{{ $label }}</span>
    @if($slot->isNotEmpty())
